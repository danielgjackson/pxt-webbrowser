...
//import Quagga from './scripts/quagga/quagga.min.js';
...
function barcodeImportScript(src) {
...
class Barcode {
    ...
    async init(initOptions) {
        ...
        if (!Barcode.loaded) {
            console.log('BARCODE: Loading barcode script...')
            await barcodeImportScript('./scripts/quagga/quagga.min.js');
            Barcode.loaded = true;
            console.log('BARCODE: ...script loaded.')
        } else {
            console.log('BARCODE: Script already loaded.')
        }
...
Barcode.scan = async (options) => {
    ...
    // Set readers 
    const readers = (options && options.readers) ? options.readers : 'code_128,ean';
